...
const AddProduct = () => {
  ...
  const [colors, setColors] = useState<string[]>(['Xanh lá cây', 'Xanh Nước biển']); // Color options
  ...
  const [showVariants, setShowVariants] = useState<boolean>(true); // State to manage variant form visibility
  const [variantImages, setVariantImages] = useState<{ [key: string]: string | null }>({}); // For variant representative images
  const [variantAlbums, setVariantAlbums] = useState<{ [key: string]: string[] }>({}); // For variant album images
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // You can add server-side submission logic here
  };
              ...
              <div className="grid grid-cols-2 gap-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex flex-col">
                    ...
                    {mainImage && (
                      <div className="mt-4">
                        <img src={mainImage} alt="Ảnh đại diện" className="w-48 h-48 object-cover rounded-lg shadow-md" />
                        <button
                          type="button"
                          className="mt-2 p-2 bg-red-500 text-white rounded-lg hover:bg-red-600"
                          onClick={() => setMainImage(null)} // Remove selected image
                        >
                          Xóa ảnh
                        </button>
                      </div>
                                ...
                                {variantImages[selectedColor] && (
                                  <div className="mt-4">
                                    ...
                                    <button
                                      type="button"
                                      className="mt-2 p-2 bg-red-500 text-white rounded-lg hover:bg-red-600"
                                      onClick={() => setVariantImages((prev) => ({ ...prev, [selectedColor]: null }))} // Remove selected image
                                    >
                                      Xóa ảnh đại diện
                                    </button>
                                  </div>
