...
const UpdateProduct: React.FC = () => {
  ...
  const { mutate: updateProduct } = useMutation({
    mutationFn: async (data: VariantProduct) => {
      const response = await axios.put(`http://localhost:8000/api/admins/products/${id}`, data);
      return response.data;
    },
    onSuccess: () => {
  ...
  const onFinish = (values: any) => {
    ...
    if (!showVariantForm) {
      ...
      updateProduct(simpleProductPayload);
      return;
    }
    ...
    console.log('Product with Variants Payload:', productWithVariantsPayload);

    updateProduct(productWithVariantsPayload);
  };
  ...
  return (
    <>
      <div className="w-full px-6 py-8">
        <ToastContainer />
        <h2 className="text-4xl font-bold mb-6">Update sản phẩm</h2>
        <Form
          form={form}
          layout="vertical"
          onFinish={onFinish}
          initialValues={{
            variant_group: UpdateVariant?.group?.id, // Set giá trị mặc định cho nhóm biến thể
            attributes: UpdateVariant?.variations?.map((variation: any) => ({
              stock: variation.stock,
              attribute_value: variation.variation_values.map((v: any) => ({
            ...
            ...UpdateVariant, // Sử dụng tất cả các trường từ UpdateVariant
          }}
        >
          <Form.Item
            label="Tên sản phẩm"
          ...
          {showVariantForm && (
            <>
              <Form
                form={form}
                initialValues={{
                  variant_group: UpdateVariant?.group?.id, // Đặt giá trị mặc định từ UpdateVariant
                }}
              >
                {/* Chọn nhóm biến thể */}
                <Form.Item
                ...
                >
                  <Select placeholder="Chọn nhóm biến thể" onChange={setSelectedVariantGroup}>
                    <Option key={UpdateVariant?.group?.id} value={UpdateVariant?.group?.id}>
                      {UpdateVariant?.group?.name}
                    </Option>
                  </Select>
                </Form.Item>
                      ...
                      <Select
                        ...
                        onChange={(values: { key: string; label: string }[]) =>
                          handleAttributeValueChange(
                            index,
                            values.map((value) => Number(value.key)) // Truyền 'id' kiểu number
                          )
                        }
                        
                        labelInValue
                      >
                        {attribute.attribute_values.map((val: { id: number; value: string }) => (
