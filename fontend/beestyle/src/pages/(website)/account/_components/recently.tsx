...
const RecentlyPage = (props: Props) => {
    ...
    const [selectedProducts, setSelectProducts] = useState<any>(JSON.parse(localStorage.getItem("showView") || '[]'))
    ...
    const handleSelectProduct = (productId: any) => {
        if (selectedProductsId.includes(productId)) {
            ...
            setSelectProductsId([...selectedProductsId, productId])
        }
    }
    const handleDeleteProducts = () => {
        if (selectedProductsId.length !== 0) {
            const updatedProducts = selectedProducts.filter((item: any) => !selectedProductsId.includes(item.id));
            localStorage.setItem("showView", JSON.stringify(updatedProducts));
            setSelectProducts(updatedProducts);
            setSelectProductsId([]);
        } else {
            localStorage.removeItem("showView");
            setSelectProducts([])
        }
        messageApi.open({
            type: "success",
    ...
    return (
        <div>
            ...
            <div className="flex justify-between py-[15px]  lg:border-b-[3px] lg:border-b-black lg:mb-[15px] lg:py-[10px]">
                <div className="text-[16px] font-[700] lg:text-[20px] ">Có {selectedProducts.length || 0} sản phẩm đã xem</div>
                <div className="text-[14px] font-[500] text-[#787878] underline">
                    <span onClick={() => { setEditShowView(!isEditShowView), setSelectProductsId([]) }} className='cursor-pointer select-none'>Chỉnh sửa</span>
                    <span onClick={() => handleDeleteProducts()} className='ml-[10px] cursor-pointer select-none'>Xóa hết</span>
                </div>
            </div>
                ...
                {selectedProducts.length > 0 ?
                    selectedProducts?.map((item: any) => (
                        <div className="mb-[30px] col-span-1 relative lg:col-span-3 lg:mb-[40px]">
                            <input className={`${isEditShowView ? "" : "hidden"} absolute w-[20px] h-[20px] z-10`}
                                type="checkbox"
                                checked={selectedProductsId.includes(item.id)}
                                onChange={() => handleSelectProduct(item.id)}
                            />
                            <div onClick={() => { setCartItem(item), setActiveCart(!activeCart) }} className="absolute cursor-pointer top-[16px] right-[16px]">
                                    ...
                                    {item.variations.map((item: any) => (
                                        <React.Fragment >
                                            ...
                                            <label htmlFor={``} className="w-[12px] h-[12px] cursor-pointer">
                                                <img className="w-[12px] h-[12px] rounded-[100%]" alt="" src={item.attribute_value_image_variant.image_path} />
                                            </label>
                                        </React.Fragment>
                                    ))}
                    ...
                    :
                    (<div className="w-[100%] pt-[150px] pb-[70px] flex flex-col items-center col-span-4 lg:col-span-12">
                        <div className="">
                        ...
                        <div className="my-[24px] text-[14px] font-[500] text-center text-[#BCBCBC]">
                            <p className=''>Không có sản phẩm nào xem gần đây</p>
                            <span>Duyệt các sản phẩm được đề xuất</span>
                        </div>
                    </div>)
                }
