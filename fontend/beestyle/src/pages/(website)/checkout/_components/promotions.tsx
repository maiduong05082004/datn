                ...
                <div className="p-[16px_32px] pt-[0] overflow-x-auto max-h-[400px]">
                    {promotions?.data.length > 0 ?
                        promotions?.data?.sort((a: any, b: any) => (b.promotion_type !== "shipping" ? 1 : 0) - (a.promotion_type !== "shipping" ? 1 : 0))
                            .map((item: any) => {
                                const isDisabled = totalPrice < item.min_order_value;
                                return (
                                    <div
                                        key={item.id}
                                        className={`${promotion.some((promo: any) => promo.promotion_type === item.promotion_type && promo.id !== item.id) || isDisabled ? "bg-slate-300 opacity-50 pointer-events-none" : ""} select-none border-[1px] border-[#787878] mb-[10px] flex items-center`}>
                                            ...
                                            <div className="p-[10px_16px] flex justify-between items-end">
                                                <div>
                                                    <ul className='text-[14px]'>
                                                        ...
                                                        <li>Đơn hàng tối thiểu: {new Intl.NumberFormat('vi-VN').format(Math.round(item.min_order_value))}đ</li>
                                                    </ul>
                                                </div>
                                            </div>
                                        </div>
